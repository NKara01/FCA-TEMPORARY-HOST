...
"""
    ...
    def deletePost(postID,owner_ID):
        ...
        if str(session["user_id"]) == str(owner_ID):
            status = 1
        elif str(session["user_id"]) != str(owner_ID) and session['role'] == 'admin':
            status = 2
...
WHERE answer_votes.voter_id = ? AND answer_votes.answer_id = ?"""
    ...
    def forumThread(threadID,page=1):
        ...
        if request.method == "POST" and "deleteQuestion" in request.form:
            ...
            deleteThread(questionID,authorID)
...
WHERE questions.id = ?"""
        ...
        question = {
            ...
            'author' : {'username': question1[4] + " " + question1[5]},
            'id' : threadID,
            'user_id' : question1[6],
            'avatar' : question1[7],
            'deleted' : question1[8],
...
WHERE question_votes.question_id = 5"""
    ...
    def profilePasswordEdit(user_id):
        ...
        if request.method == "POST" and str(user_id) == str(session.get('user_id')):
            ...
            if bcrypt.checkpw(entered_password_byte, stored_hash_bytes):
                ...
                messages["old_password"] = "Password does not match"
    ...
    def profile(user_id):
        ...
        conn, cursor = get_db()
        cursor.execute("SELECT id,first_name,last_name,email,created_at,bio,avatar FROM users WHERE users.id = " + str(user_id) + ";")
        ...
        if request.method == "POST" and str(user_id) == str(session.get('user_id')):
            ...
            avatar = "assets/avatars/" + f.filename
            if 'file' not in request.files or f.filename == '':
                ...
                f.save("frontend/assets/avatars/" + f.filename)  
  ...
  END AS "not_purchased"
        ...
        for m in modules:
            if not m["image_url"] or m["image_url"] == "NIL":
                m["image_url"] = "../frontend/assets/defaults/default_module_image.png"
...
WHERE modules.id = ? AND module_parts.id = ?"""
        ...
        close_db()
        return render_template("module_components/module_part.html",content = content,module_list = module_list,final_quiz = final_quiz, canProceed = canProceed,navigator = navigator)
...
WHERE module_answers.question_id = ?"""
    ...
    def moduleExam(module_id):
        ...
        query = """SELECT *
FROM module_quizzes
WHERE module_quizzes.module_id = ? AND module_quizzes.part_id IS NULL AND module_quizzes.is_final_exam"""
    ...
    def newsArticle(newsId):
        ...
        if request.method == "POST" and "editNews" in request.form:
            ...
            if 'file' not in request.files or f.filename == '':
                ...
                f.save("frontend/assets/news/" + f.filename)  
...
"""
    ...
    def deleteBlogComment(commentID,ownerID):
        ...
        if str(session["user_id"]) == str(ownerID):
            status = 1
        elif str(session["user_id"]) != str(ownerID) and session['role'] == 'admin':
            status = 2
        ...
        if request.method == "POST":
            ...
            if form_type == "role":
                ...
                if str(target_id) == str(session['user_id']):
                    ...
                    conn.commit()
                    # string formatting for message when changing roles in admin page
 ...
 # ---------------------------------------------------------------------------------------------------
    ...
    def register_module_admin_routes(app):
        ...
        def api_create_module():
            ...
            image = "../frontend/assets/defaults/default_module_image.png"
            if f and f != "":
                image = "assets/modules/" + f.filename
                print("FILENAME:",f.filename)
                Path("frontend/assets/modules").mkdir(parents=True, exist_ok=True)
                f.save("frontend/assets/modules/" + f.filename)  
