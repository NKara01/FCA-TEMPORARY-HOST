...
def validate_password_only(password,password_2):
    ...
    if not mat: 
        error_message = "Password need to be 6 to 20 characters inclusive, with 1 upper and lowercase, 1 number and 1 special character."   
...
def validate_credentials(email, password, password_2, first_name, last_name):
    ...
    mat = re.search(pat, password)
    if not mat: 
        error_message = "Password does not match criteria"   
        return False, error_message
    if password != password_2: 
        error_message = "Passwords do not match"
        return False, error_message
